...
function editorUpdate(dt)
	...
	elseif editorMode == 1 then
		...
		if editorModeScreen == 1 then
			mx = _f(ms.getX() + editorScrollX)
			my = _f(ms.getY() + editorScrollY)
		...
		local mms = 512*dt
		if ms.isDown("r") then mms = 512*dt end
		local rmd = ms.isDown("r")
        ...
        editorHoverScenery = found
			...
			if kb.isDown("lshift") == false and kb.isDown("lalt") == false and kb.isDown("lctrl") == false then
				if kLeft or (rmd and ms.getX() < 150) then
					editorScrollX = editorScrollX - mms
				elseif kRight or (rmd and ms.getX() > 800-150) then
					editorScrollX = editorScrollX + mms
				end
				if kUp or (rmd and ms.getY() < 150) then
					editorScrollY = editorScrollY - mms
				elseif kDown or (rmd and ms.getY() > 500-150) then
					editorScrollY = editorScrollY + mms
				end
			end
			...
			elseif editorMouseTouch == 2 then
				editorToolBox.x = ms.getX() - editorToolBox.cxo
				editorToolBox.y = ms.getY() - editorToolBox.cyo
	...
	elseif editorMode == 2 then
		...
		if editorModeScreen == 1 then
          ...
          if y < 0 then y = 0 end
			...
			gr.setColor(255,255,255,255)
			gr.print(data, editorToolBox.x+6, editorToolBox.y)
		...
		elseif editorModeScreen == 3 then
			...
			gr.setColor(0,0,0)
			gr.setColorMode("modulate")
			gr.printf("Scenery: " .. sl .. "\nWidth: " .. w .. "  Height: " .. h .. "\nOffset X: " .. sceneryLibrary[sl].ox .. "  Y: " .. sceneryLibrary[sl].oy .. an, screenW - 650, 10, 600, "right")
			...
			for i=editorLibraryListScroll,m do
				...
				gr.print(t, 40, 15+(editorLibraryListTextHeight*(i-editorLibraryListScroll)))
			end
		elseif editorModeScreen == 4 then
			...
			gr.setColor(255,255,255)
			gr.drawq(menuTitle, imgLogoBackground, 0, 0)
			gr.print("Set Background Color:",20,20)
			gr.print("Up/Down: Select		Left/Right: Change",20,470)
